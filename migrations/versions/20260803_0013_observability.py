"""Create operational incident, heartbeat, and SLO records."""

from collections.abc import Sequence
from typing import cast

from alembic import op
from sqlalchemy import Table

from apps.api.app.observability.models import (
    IncidentTimelineEntry,
    OperationalIncident,
    ServiceHeartbeat,
    SLORecord,
)

revision = "20260803_0013"
down_revision: str | Sequence[str] | None = "20260803_0012"
branch_labels = depends_on = None
TABLES = tuple(
    cast(Table, model.__table__)
    for model in (OperationalIncident, IncidentTimelineEntry, ServiceHeartbeat, SLORecord)
)


def upgrade() -> None:
    for table in TABLES:
        table.create(bind=op.get_bind(), checkfirst=False)


def downgrade() -> None:
    for table in reversed(TABLES):
        table.drop(bind=op.get_bind(), checkfirst=False)
