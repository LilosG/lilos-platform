...
| GBP operational product           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | sync/media/post/agent workflows partial          | partial                  | **PR #47:** The versioned GBP Hermes skill reads approved facts, website knowledge, profile state, provider/LILOs post history, and creates only approval-waiting Local Post or optimization proposals through canonical GBP services. Google publication remains exclusively LILOs workflow-controlled. Repository acceptance passes; deployed agent execution and a real governed proposal remain unaccepted.                                                                                      |
| Reviews                           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | ingestion/reply/agent workflow partial           | partial                  | **PR #47:** The Reviews Hermes skill reads persisted review state and approved facts, treats deterministic restricted-risk classification as a hard denial, and creates only human-approval response revisions through the canonical Reviews service. Live agent drafting and provider publication remain unaccepted.                                                                                                                                                                                |
...
| SEO                               | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | deterministic analysis + agent proposals partial | partial                  | **PR #47:** Deterministic crawl/GSC/PageSpeed opportunity production remains authoritative. The SEO Hermes skill may correlate persisted GSC/GA4/Content evidence, request the canonical crawl workflow, and create evidence-linked recommendation or accepted-opportunity Content proposals; it has no production-site mutation tool. Live evidence-backed agent acceptance remains pending.                                                                                                        |
| Content                           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | proposal/brief/draft agent workflow partial      | partial                  | **PR #47:** The Content Hermes skill converts accepted opportunities into canonical items, requires current approved facts and source references for ready briefs, and creates grounded AI revisions linked to the owning `AIExecution`; editorial/client approval and GitHub publication remain canonical LILOs workflows. Live agent and publication acceptance remain pending.                                                                                                                    |
...
| Automation & Agents control plane | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | IMPLEMENTED_NOT_ACCEPTED                         | IMPLEMENTED_NOT_ACCEPTED | **PR #47:** Five native Hermes agent workflows reuse the existing LILOs catalog/worker/scheduler. Runs bind to workflow/AI execution, ingest bounded structured events, reconcile known IDs, expose capability-gated stop/real HTTP steer/approval, retain scoped session continuity/reset, and render history/tool/proposal/audit evidence in Automations. Hermes jobs are unused. Repository acceptance passes; live production acceptance remains pending.                                        |
| Production AI provider            | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #47:** Hermes is upgraded to exact image v2026.8.19 and digest `sha256:3811ed…ccec09` (runtime 0.20.5) for genuine native HTTP run steer. Local exact-image probes advertise all eight required Runs features and exactly the 20-tool `lilos` toolset with no other enabled toolset. Private deployed parity and real model execution remain unaccepted.                                                                                                                                        |
| AI Gateway production routing     | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #47:** Agent-grade work uses authenticated native Runs create/status/SSE/approval/steer/stop rather than mislabeled follow-ups. Capability/runtime/toolset mismatch fails closed. Existing one-shot GBP/Content/Reviews paths remain regression-supported. Live routed execution and deployed API/worker/Hermes skew checks remain pending.                                                                                                                                                     |
...
| Insights / reporting              | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | scheduled cross-product agent partial            | IMPLEMENTED_NOT_ACCEPTED | **PR #47:** `agent.insights` is an inspectable location-scoped Hermes run over persisted GBP, GSC, GA4, Reviews, Content, crawl/SEO, workflow, and approved-fact evidence. It reports changes, evidence-backed drivers, attention, and linked actions without converting missing data to zero. One real cross-product production run remains required.                                                                                                                                               |
| Release/production acceptance     | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #47:** migration head advances to `20260824_0002`. Focused native protocol, tool/security, tenant/session, migration, product-regression, web, Render-policy, exact pinned-image probes, and diagnosed integrated repository acceptance are green. Live private run/tool/proposal/approval/stop/steer/isolation/audit/release-parity acceptance remains required before this release-closure packet can be accepted.                                                                            |
