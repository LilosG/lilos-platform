...
| Entitlement-aware navigation      | PILOT_READY                 | n/a             | IMPLEMENTED_NOT_ACCEPTED | n/a                                              | n/a                      | **Packet 1/RR-1:** Navigation remains role-aware via `_updateAdminNavigation()`. RR-1 now makes effective organization/location product entitlement conjunctive with the existing backend user, membership, role-permission, location, AAL, catalog, and explicit-deny checks. Missing, not-enabled, suspended, archived, future, expired, unknown, and wrong-location entitlements fail closed; entitlement never grants a role permission. Focused and integrated repository acceptance passed, so RR-1 is repository `PILOT_READY`; live client-role/entitlement acceptance remains pending. |
| Unified onboarding                | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | n/a                                              | readiness partial        | **Packet 2 + 2026-09-01 completion pass:** Three responsibility modes remain on one engine. Source-first onboarding now auto-establishes profile/owner, permits read-only Google/GBP setup before activation, provisions the website crawl from the primary domain, reconciles source-backed facts, and provides location-profile editing. Only core client identity details block activation; product readiness remains truthful and non-blocking. Blockers deep-link to working controls. Repository validation complete; live source/provider acceptance pending.                            |
...
| Google provider resource mapping  | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | n/a                                              | n/a                      | **PR #44:** Integrations now consumes canonical, connection-scoped `GBPLocation` identity and write-governance truth; mapped/unmapped reconciliation uses provider-mapping and platform `Location.id` identities consistently. The existing AAL2 GBP confirm mutation remains the sole mapping/write-access mutation. PR44 integrated repository acceptance passed after merging PR45; live post-deploy control-plane acceptance remains outstanding.                                                                                                                                           |
| GBP operational product           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | sync/media/post/agent workflows partial          | partial                  | **PR #46/PR #47:** PR46's capability-aware worker preflight, duplicate-safe publication evidence, fail-closed dispatch ambiguity, governed AAL2 recovery, truthful publication reads, and `LIVE`-only provider counts remain repository `PILOT_READY`. PR47 adds the governed GBP Hermes skill over approved facts and canonical GBP reads/proposals; Google publication remains exclusively LILOs workflow-controlled. Deployment, governed stranded-publication recovery, a real Google `LIVE` post, deployed agent execution, and a real governed proposal remain unaccepted.                |
| Reviews                           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | ingestion/reply/agent workflow partial           | partial                  | **Packet 4/PR #47:** The compact operational inbox and approval/publish stages remain in place. PR47 adds a Reviews Hermes skill over persisted reviews and approved facts; deterministic restricted-risk classification is a hard denial, and the agent can create only human-approval response revisions through the canonical Reviews service. Live agent drafting and provider publication remain unaccepted.                                                                                                                                                                               |
...
| SEO                               | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | deterministic analysis + agent proposals partial | partial                  | **PR #39/PR #47:** PR39's deterministic newest-period precedence, unmapped-demand proof, SEO/Content routing, and stale-opportunity archival remain authoritative. PR47 adds a Hermes skill that correlates persisted GSC/GA4/Content evidence, requests the canonical crawl workflow, and creates evidence-linked recommendations or accepted-opportunity Content proposals; it has no production-site mutation tool. Live crawl/GSC/recommendation lifecycle and evidence-backed agent acceptance remain pending.                                                                             |
| Content                           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | proposal/brief/draft agent workflow partial      | partial                  | **PR #39/PR #47:** PR39's content-addressable SEO routing, indexed tenant-scoped deduplication, governed-fact grounding, and durable AI drafting remain in place. PR47 adds a Hermes skill that converts accepted opportunities into canonical items, requires current approved facts and source references for ready briefs, and creates grounded AI revisions linked to the owning `AIExecution`; editorial/client approval and GitHub publication remain canonical LILOs workflows. Live agent and publication acceptance remain pending.                                                    |
...
| Integrations control plane        | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | n/a                                              | health partial           | **Packet 4/PR #44:** Integrations owns connection health, capabilities, mapped-resource freshness, privileged discovery/mapping, and deliberate provider-write governance. Successful discovery, mapping, and write-access changes now re-fetch connection/workspace backend truth. PR44 integrated repository acceptance passed after merging current main. Connected-account label, per-capability sync timestamps, and a manual sync endpoint remain absent and were not fabricated.                                                                                                         |
| Automation & Agents control plane | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | IMPLEMENTED_NOT_ACCEPTED                         | IMPLEMENTED_NOT_ACCEPTED | **PR #39/PR #46/PR #47:** Duplicate registration, capability-aware worker startup, and auditable duplicate-safe recovery remain on the existing `workflow.execute` runtime. PR47 adds five native Hermes agent workflows on the same catalog/worker/scheduler; runs bind to workflow/AI execution, project bounded events, reconcile known IDs, expose capability-gated stop/real HTTP steer/approval, and retain scoped session continuity/reset. Hermes jobs are unused. Live recovery and agent acceptance remain pending.                                                                   |
| Production AI provider            | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #39/PR #47:** Hermes remains the production-primary private runtime boundary with fail-closed adapter contracts. PR47 upgrades the exact image to v2026.8.19 and digest `sha256:3811ed…ccec09` (runtime 0.20.5) for genuine native HTTP run steer; local probes advertise all eight required Runs features and exactly the 20-tool `lilos` toolset with no other enabled toolset. Private deployed parity and real model execution remain unaccepted.                                                                                                                                      |
| AI Gateway production routing     | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #39/PR #47:** Provider-neutral routing continues to pass authoritative scope, derive opaque tenant-scoped sessions, reject secret-bearing inputs, and preserve usage bounds/history. PR47 routes agent-grade work through authenticated native Runs create/status/SSE/approval/steer/stop, with capability/runtime/toolset mismatch failing closed; existing one-shot GBP/Content/Reviews paths remain regression-supported. Live routed execution and deployed API/worker/Hermes skew checks remain pending.                                                                              |
...
| Insights / reporting              | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | scheduled cross-product agent partial            | IMPLEMENTED_NOT_ACCEPTED | **Packet 4/PR #47:** The period/context, comparison, source/freshness, and non-interpolated-null reporting contract remains in place. PR47 adds `agent.insights`, an inspectable location-scoped Hermes run over persisted GBP, GSC, GA4, Reviews, Content, crawl/SEO, workflow, and approved-fact evidence; it reports changes, evidence-backed drivers, attention, and linked actions without converting missing data to zero. One real cross-product production run remains required.                                                                                                        |
| Release/production acceptance     | IMPLEMENTED_NOT_ACCEPTED    | partial         | n/a                      | n/a                                              | n/a                      | **PR #46/PR #47/RR-1:** PR46 and RR-1 remain independently repository `PILOT_READY`. PR47 advances the migration head to `20260824_0002`; its focused native protocol, tool/security, tenant/session, migration, product-regression, web, Render-policy, exact pinned-image probes, and diagnosed integrated repository acceptance are green, while live private run/tool/proposal/approval/stop/steer/isolation/audit/release-parity acceptance remains required. Deployed database and API/web/worker parity plus live RR-1 client-role/entitlement acceptance also remain pending.           |
...
- State: `PILOT_READY` (integrated repository acceptance complete; live provider write/publish acceptance pending).
- Integration baseline: merged `origin/main` at `c4505108f5d035c189a31209711145af27a9dbdb` without conflicts. The merge preserves PR44 provider-write governance and includes PR45's deterministic scheduler hour-boundary correction.
...
- Focused evidence: after merging current main, 24 backend/route regressions passed against isolated PostgreSQL and all 31 directly related GBP/Integrations web regressions passed.
- Integrated acceptance: the single post-merge `npm run check` invocation passed: Prettier and Ruff formatting; ESLint, Stylelint, and Ruff lint; Astro diagnostics across 148 files; mypy across 487 files; 272/272 Vitest tests; 940/940 pytest tests (including PR45's corrected scheduled-execution coverage); production build of 14 pages; and the repository secret scan.
...
### PR #46 — GBP Publication Runtime Recovery + Truthful State

- State: `PILOT_READY` (focused and integrated repository acceptance complete). Live production acceptance remains explicitly pending until the corrected release is deployed, the stranded publication is recovered through the governed action, and a real Google Local Post is provider-observed as `LIVE`.
- Production root causes confirmed:
  - `lilos-worker` previously validated only database availability before entering the durable runtime, while enabled Google provider workflows also require the encryption key and three Google OAuth values. Blueprint `sync: false` declarations did not prove those values existed in the worker environment.
  - `gbp.publish_post` moved pre-create token/secret/configuration failures into `reconciliation_required`; the next attempt then treated the missing provider identity as an ambiguous write even though the provider create boundary had not been crossed.
  - The managed Posts read path returned revision state without its publication, so an approved revision continued to render ordinary Publish after a publication existed and browser-generated timestamp keys changed across attempts.
  - Local Post summaries used persisted presence rather than provider state, so `PROCESSING`/other non-live rows could be described as visible/live.
  - The shared inline confirmation placed message plus two buttons into a three-column notice grid and could overlap at narrow widths.
- Runtime and state-machine correction:
  - Worker preflight is capability-aware and fails before the runner/heartbeat when provider writes are enabled but required Google runtime configuration is absent. Its bounded structured error names only missing environment keys and never values.
  - `GBPPostPublication.dispatched_at` and `safe_error_code` provide durable dispatch evidence. OAuth/token/secret failures remain `reserved` with a safe code; `dispatched_at` is committed immediately before `create_local_post`; provider identity is committed before verification; known provider IDs are re-read exactly; and a crossed dispatch boundary without identity remains ambiguous and cannot create again.
  - Migration `20260824_0001` adds the two nullable evidence columns and is replay-safe with the repository's historical metadata-driven GBP table migration.
- Governed recovery:
  - The location-scoped `gbp.publish` AAL2 recovery endpoint invokes canonical Local Post reconciliation first and emits success/denial audit evidence.
  - Current rows retry create-capable work only with explicit pre-dispatch safe codes. The production-era legacy sequence is recoverable only when completed job attempts prove an initial token/configuration failure followed solely by the historical ambiguous result. A durable dispatch timestamp blocks create-again.
  - A provider resource can be attached automatically only from one exact present content/type match whose provider `createTime` falls within the durable dispatch window; missing or multiple proof remains operator attention. Accepted recovery requeues the original run through the existing `workflow.execute` job path.
- Product/read truth:
  - Managed post revisions now include the latest publication and backend-calculated recovery eligibility. The Posts surface distinguishes awaiting approval, never submitted, queued, dispatched, provider processing/uncertain, verified, failed/rejected, cancelled, and expired states; any publication suppresses ordinary Publish.
  - Publish disables synchronously, reuses a revision-stable idempotency key through workflow reservation and publication reservation, and preserves action status across rerender. Recovery is shown only when the backend permits it.
  - Provider counts now distinguish `LIVE`, transitional processing, `REJECTED`, and historical `not_seen` truth. Only present `LIVE` rows are labeled live on Google; reconciliation remains idempotent and retains history.
  - Shared inline confirmation now groups actions into one responsive grid cell and stacks safely at narrow widths.
- Focused validation evidence: 4/4 worker-preflight tests; 11/11 PostgreSQL publication handler, provider-transaction boundary, retry, exact reread, provider-state, read-model, duplicate suppression, recovery AAL2/tenancy/audit/denial/match, and reversible-migration tests; 6/6 Vitest publication/count/DOM tests; 2/2 Playwright responsive confirmation tests. Targeted Ruff, mypy, ESLint, Stylelint, Prettier, and Astro diagnostics pass.
- Integrated validation: PASS in the explicitly authorized continuation `npm run check` invocation after the order-independent logger-test correction: Prettier and Ruff formatting; ESLint, Stylelint, and Ruff lint; Astro diagnostics across 151 files; mypy across 488 files; 278/278 Vitest tests; 948/948 pytest tests; production build of 14 pages; and the repository secret scan. The earlier in-scope logger-test failure and its focused correction remain recorded in repository history; no validation stage was rerun within this continuation gate.
- Safety evidence: no production SQL or provider mutation was performed; tenant/location scope, AAL2, write-enabled mapping, approval, audit, idempotency, provider verification, and reconciliation gates remain authoritative.
- Adjacent work intentionally not implemented: no Wheyland-specific IDs/content; no second orchestration framework; no direct production repair; no deployment/provider configuration mutation; no claim of live provider acceptance.
- Ledger rows changed: GBP operational product; Automation & Agents control plane; Release/production acceptance.

---

### PR #47 — Hermes Agent Operationalization / Cross-Product Closure

- State: `IMPLEMENTED_NOT_ACCEPTED`. Repository implementation and acceptance are complete; every
  live production acceptance scenario remains required.
- Confirmed pre-change gaps: pinned Hermes v2026.8.3 had no safe HTTP Runs steer; LILOs used the
  OpenAI-compatible completion surface for production AI and had no native product-agent run
  binding, event read model, real steer/stop/approval UX, sanctioned LILOs plugin toolset,
  cross-product Insights agent, or actual scoped Hermes transcript continuity.
- Runtime/protocol: exact image
  `nousresearch/hermes-agent:v2026.8.19@sha256:3811ed13da874fba2ac99b6d492db9a203d34cb6dccf90d886948c00d0ccec09`
  (OCI revision `fcbd1076a93841fa88855acce810e342a5b78101`, runtime `0.20.5`). Native
  authenticated protocol is Runs create, status, SSE events, approval response, same-process HTTP
  run steer, stop, and session delete/reset. Positive probes require all eight advertised Runs
  features, server-agent/server-tool runtime, no split runtime, and exactly one enabled `lilos`
  toolset.
- Architecture: `agent.gbp`, `agent.seo`, `agent.content`, `agent.reviews`, and `agent.insights`
  reuse the existing workflow catalog, durable worker/job leases, retries, scheduler, audit, AI
  execution, approvals, provider writes, and reconciliation. Hermes jobs/cron are not used.
- Sanctioned plane: a supported Hermes custom plugin exposes exactly 20 server-side tools for
  approved facts/knowledge; GBP/GSC/GA4/Reviews/Content/cross-product/SEO/workflow reads; canonical
  crawl request; and governed SEO, Content, GBP, Reviews, and approval proposals. Scope comes only
  from the opaque bound LILOs run. No model-visible tenant IDs, arbitrary MCP, terminal, secrets,
  credentials, Google/GitHub tool, or direct provider mutation exists.
- Session/event policy: one opaque organization/location/skill transcript plus long-term-memory
  key, database-enforced single active run, native-ID reconnect, fail-closed ambiguous-create
  rotation, scoped session deletion/reset, 30-day retention, and bounded safe lifecycle/tool/result
  projection. Reasoning, message deltas, private subagent content, secret-bearing values, and
  chain-of-thought are discarded.
- Product paths: GBP proposal/optimization, deterministic-evidence SEO recommendation/routing,
  grounded Content item/brief/revision, risk-gated Review response, and inspectable cross-product
  Insights analysis. All client/provider mutations remain human-approved canonical LILOs work.
- Focused repository evidence: 44 protocol/safety/provider/Render-policy tests plus 63
  database/product/workflow tests (including 10 PostgreSQL migration/lifecycle/tenant/session
  tests) pass; targeted Ruff and strict mypy are clean; Alembic drift is clean; 2 targeted web unit
  tests, Astro diagnostics across 153 files, and all 220 browser tests pass. The final exact image
  positively reports runtime 0.20.5, all eight required Runs features, server-agent/server-tool
  execution without split runtime, session auto-prune/30-day retention, and only the exact 20-tool
  `lilos` toolset enabled. Proposal evidence is restricted to references observed through
  sanctioned tools in the same bound run.
- Integrated repository evidence: PASS by diagnosed continuation. Prettier and Ruff format pass;
  ESLint, Stylelint, and Ruff lint pass; Astro diagnostics cover 153 files; mypy covers 503 files;
  280/280 Vitest tests pass; the Python run completed 967/972 before five exact PR47-owned
  stale/order assertions, and all five corrections pass in a 6/6 focused sequence (including
  Alembic-before-logger order); the 14-page production build and secret scan pass. Per the packet,
  the full Python validation was not run a third time after the same stage failed twice.
- Live acceptance: not claimed. Required after deploy: private run creation; visible tool events;
  approved facts plus real GBP and GSC/GA4/crawl reads; one cross-product result; one governed GBP
  or Content proposal; human approval; live stop; genuine HTTP run steer; inability to initiate raw
  provider writes; complete workflow/audit evidence; second-tenant isolation; and visible
  Render/API/worker/Hermes release parity.
- Ledger rows changed: GBP operational product; Reviews; SEO; Content; Automation & Agents control
  plane; Production AI provider; AI Gateway production routing; Insights / reporting;
  Release/production acceptance.
- Adjacent work intentionally not implemented: none is deferred as “future Hermes
  operationalization.” Live deployment/provider acceptance is an acceptance gate, not a deferred
  implementation packet. Unrelated provider/product defects remain under their existing owners.

### RR-1 — Backend Product-Entitlement Authorization

- State: `PILOT_READY`. Focused and integrated repository acceptance are green; live
  client-role/entitlement acceptance remains pending.
- Root cause confirmed: the shared authorization decision service evaluated active user,
  organization, membership, location ownership, AAL, role-permission grants, catalog consistency,
  and explicit denies, but did not evaluate `product_entitlements`. A caller holding a product
  permission could therefore call the API for an unentitled product directly.
- Mapping strategy: one exact immutable permission-to-product table covers every registered GBP,
  Reviews, Leads, Content, SEO, and Insights permission. It never derives product identity from
  route text or arbitrary string prefixes, and a contract test requires the table to remain
  complete against the access and product catalogs.
- Enforcement: entitlement evaluation occurs only after the existing role grants the permission
  and after explicit-deny evaluation, so entitlement can restrict but never grant authority. One
  organization-scoped query loads the product, entitlement lifecycle/effective period, and all
  location rows. Missing, `not_enabled`, `suspended`, `archived`, future, expired, unknown,
  removed-location, wrong-location, and catalog-inconsistent states fail closed.
  Organization-wide entitlements remain applicable to location requests; location-restricted
  entitlements require the exact active location and cannot authorize organization-wide requests.
- Platform-administrator contract: the existing active-grant/AAL platform-administration
  dependency remains separate from organization RBAC. Its legitimate platform routes remain
  available, while a platform-administrator grant alone does not bypass organization membership,
  role permission, or product entitlement for operational product routes.
- Focused validation: Ruff format/lint and mypy passed for touched backend and
  authorization-local fixture files; 17/17 authorization tests passed against isolated
  PostgreSQL; 50/50 affected GBP, Reviews, SEO, Content, and Leads API regressions passed against
  isolated PostgreSQL.
- Integrated validation: PASS in the authorized continuation after `npm ci` installed the locked
  Node dependencies: the continuation's single `npm run check` passed Prettier and Ruff formatting
  (494 Python files), ESLint, Stylelint, and Ruff lint, Astro diagnostics across 151 files, mypy
  across 490 files, 278/278 Vitest tests, 477 passed and 474 skipped pytest tests, the 14-page
  production build, and the repository secret scan. The prior exit 127 caused solely by absent
  worktree dependencies remains recorded in packet history; no integrated stage was rerun within
  this continuation gate.
- Live acceptance: real client-role/entitlement behavior remains pending after deployment and
  controlled live verification.
- Adjacent work intentionally not implemented: no route, product service, workflow/execution,
