...
| GBP operational product | IMPLEMENTED_NOT_ACCEPTED | partial | IMPLEMENTED_NOT_ACCEPTED | sync/media/post workflows partial | partial | **PR #39:** Profile and post publication now use one canonical handler per workflow, release database locks before provider I/O, preserve dispatched identities for reconciliation, and enqueue resource-bound runs only after the authoritative resource exists. Governed-fact GBP drafting and canonical media publication are covered by focused tests. Live provider read/write acceptance remains outstanding. |
