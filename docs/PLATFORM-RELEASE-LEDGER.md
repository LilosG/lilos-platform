...
| GBP operational product           | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | sync/media/post workflows partial   | partial                  | **PR #39/PR #44/PR #46:** PR46 corrects the production publication-runtime failure class platform-wide: the worker now fails startup when enabled Google writes lack runtime configuration; pre-dispatch token/configuration failures retain duplicate-safe retry evidence; actual dispatch ambiguity remains fail-closed; AAL2 recovery reconciles first and reuses the canonical workflow queue; managed revisions expose publication truth and suppress duplicate Publish; and provider summaries count only `LIVE` rows as live. PR46 focused and integrated repository acceptance passed and the packet is `PILOT_READY`; deploy, governed stranded-publication recovery, and a real Google `LIVE` post remain unaccepted. |
...
| Automation & Agents control plane | IMPLEMENTED_NOT_ACCEPTED    | partial         | IMPLEMENTED_NOT_ACCEPTED | IMPLEMENTED_NOT_ACCEPTED            | IMPLEMENTED_NOT_ACCEPTED | **PR #39/PR #46:** Duplicate workflow registration remains fail-closed and PR46 adds capability-aware worker startup validation plus an auditable recovery enqueue on the existing `workflow.execute` job/runtime path. Recovery locks the original run, does not create a parallel orchestrator, and only resumes create-capable work from durable pre-dispatch evidence. Live production recovery remains unaccepted.                                                                                                                                                                                                                                                                                                         |
...
| Release/production acceptance     | PILOT_READY                 | partial         | n/a                      | n/a                                 | n/a                      | **PR #46/RR-1:** PR46 remains repository `PILOT_READY` on its accepted commit. For RR-1, the locked Node dependencies installed successfully and the authorized continuation's single `npm run check` passed all formatting, lint, typecheck, test, build, and secret-scan stages. RR-1 is repository `PILOT_READY`; deployed database and API/web/worker parity plus live client-role/entitlement acceptance remain pending.                                                                                                                                                                                                                                                                                                   |
...
- State: `PILOT_READY`. Focused and integrated repository acceptance are green; live client-role/entitlement acceptance remains pending.
- Root cause confirmed: the shared authorization decision service evaluated active user, organization, membership, location ownership, AAL, role-permission grants, catalog consistency, and explicit denies, but did not evaluate `product_entitlements`. A caller holding a product permission could therefore call the API for an unentitled product directly.
- Mapping strategy: one exact immutable permission-to-product table covers every registered GBP, Reviews, Leads, Content, SEO, and Insights permission. It never derives product identity from route text or arbitrary string prefixes, and a contract test requires the table to remain complete against the access and product catalogs.
- Enforcement: entitlement evaluation occurs only after the existing role grants the permission and after explicit-deny evaluation, so entitlement can restrict but never grant authority. One organization-scoped query loads the product, entitlement lifecycle/effective period, and all location rows. Missing, `not_enabled`, `suspended`, `archived`, future, expired, unknown, removed-location, wrong-location, and catalog-inconsistent states fail closed. Organization-wide entitlements remain applicable to location requests; location-restricted entitlements require the exact active location and cannot authorize organization-wide requests.
- Platform-administrator contract: the existing active-grant/AAL platform-administration dependency remains separate from organization RBAC. Its legitimate platform routes remain available, while a platform-administrator grant alone does not bypass organization membership, role permission, or product entitlement for operational product routes.
- Focused validation: Ruff format/lint and mypy passed for touched backend and authorization-local fixture files; 17/17 authorization tests passed against isolated PostgreSQL; 50/50 affected GBP, Reviews, SEO, Content, and Leads API regressions passed against isolated PostgreSQL.
- Integrated validation: PASS in the authorized continuation after `npm ci` installed the locked Node dependencies: the continuation's single `npm run check` passed Prettier and Ruff formatting (494 Python files), ESLint, Stylelint, and Ruff lint, Astro diagnostics across 151 files, mypy across 490 files, 278/278 Vitest tests, 477 passed and 474 skipped pytest tests, the 14-page production build, and the repository secret scan. The prior exit 127 caused solely by absent worktree dependencies remains recorded in packet history; no integrated stage was rerun within this continuation gate.
- Live acceptance: real client-role/entitlement behavior remains pending after deployment and controlled live verification.
- Adjacent work intentionally not implemented: no route, product service, workflow/execution, Hermes/AI, integrations, onboarding, or product implementation changes.
